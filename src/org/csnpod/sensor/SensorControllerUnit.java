...
public class SensorControllerUnit extends Thread {
	...
	private void initSensorThreads() {
		...
		for (PhysicalSensorInformation metadata : SensorUnitConfig.physicalSnsrInfo) {
			...
			System.out.println(metadata.getSensorType()+"   "+metadata.getPort());
			Sensor sensorThread = new Sensor(metadata.getName(),metadata.getSensorType(),metadata.getPort(),sensorDataQueue, metadata);
